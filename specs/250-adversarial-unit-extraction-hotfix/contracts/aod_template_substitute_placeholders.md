...
Each `{{PROJECT_NAME}}` (etc.) token in the source is literally replaced with the value of `AOD_PERSONALIZATION_PROJECT_NAME` (etc.). Replacement is byte-literal — no regex interpretation, no escape processing.
...
- Case 1 (`AT&T`) to corrupt to `AT{{PROJECT_NAME}}T` (the F-248 closure)

...
**Accepted-commit-SHA**: `c4b8dc68f36b59ee7ab49cc587661526ffd1a818`
...
**2026-04-18 (Accepted — Feature 194, T044 post-merge SHA fill)**: PR #195 squash-merged to `main` at 2026-04-18 18:28:36 UTC with commit SHA `c4b8dc68f36b59ee7ab49cc587661526ffd1a818`. `Accepted-commit-SHA` field on line 5 updated from `<pending-post-merge-fill>` placeholder to the actual squash-merge SHA per Decision 7 dual-commit governance protocol and F-A1 / F-A2 (ADR-027 T039 / ADR-028 T036) precedent. **Provisional Accepted-date 2026-04-23 retained** (not corrected to actual merge date 2026-04-18) per ADR-027 T039 / ADR-028 T036 guidance: the provisional date reflects the Wave 4.2 T043 authoring-time projection, and the paired `<pending-post-merge-fill>` placeholder-replaced-by-SHA satisfies the "provisional date AND SHA-fill post-merge" pattern; correcting the date would retcon the Wave 4.2 T043 transition record without adding provenance value beyond what the new SHA field now provides. Merge occurred 5 calendar days earlier than the provisional Accepted-date — the 4-day team-lead plan envelope compressed to a single-day autonomous execution per KB-030 compressed-delivery pattern. A mid-merge fix (commit `6136e6b`, pre-squash) deferred `import yaml` inside `_load_framework_yaml_records` to preserve the Feature 128 stdlib-only module-load invariant and unblock the Feature 130 mmdc preflight CI gate — captured as KB-037 in `docs/INSTITUTIONAL_KNOWLEDGE.md`, does not affect any of the 7 Decisions. This entry closes the T044 post-merge-fill dependency; all 7 Decisions are carried forward unchanged. No substantive content updates; provenance-strengthening only.

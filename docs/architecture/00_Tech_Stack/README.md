...
|--------|---------|------------|
| `schemas/finding.yaml` | Intermediate Representation (IR) -- data contract between agents and templates; optional `scored_finding` extension for quantitative risk scoring (Feature 035); baseline-aware fields `delta_status` and `baseline_run_id` (Feature 074); MAESTRO layer classification `maestro_layer` (Feature 084); canonical CSA layer names for L5/L6/L7 (Feature 136); agentic pattern classification `agentic_pattern` (Feature 142); optional list-of-RECORD `source_attribution` field citing F-A1 external-framework IDs (Feature 189 F-A2) | 15 fields: id, category, component, threat, likelihood, impact, risk_level, mitigation, references, dfd_element_type, maestro_layer (L1-L7 canonical CSA names or Unclassified, default Unclassified — L5 Evaluation and Observability, L6 Security and Compliance, L7 Agent Ecosystem per Feature 136), delta_status (NEW/UNCHANGED/UPDATED/RESOLVED), baseline_run_id (nullable), **agentic_pattern** (8-value enum per Feature 142: 6 canonical CSA MAESTRO patterns `agent_collusion`, `emergent_behavior`, `temporal_attack`, `trust_exploitation`, `communication_vulnerability`, `resource_competition` + `none` + `multiple`, default `none`), **source_attribution** (optional list-of-RECORD per Feature 189; each record `{taxonomy, id, relationship}` with closed 5-value `taxonomy` enum — `owasp` / `mitre-attack` / `mitre-atlas` / `nist-ai-rmf` / `cwe` external frameworks only — and closed 3-value `relationship` enum — `primary` / `related` / `derived` with default `primary`; field is absent when a finding cites no framework items; F-A2 ships the contract only, populator wiring deferred to F-A3 under 22-file zero-edit invariant); `id.pattern` regex extended in Feature 142 to accept `AGP-\d+` prefix for net-new findings emitted by the Phase 3.6 synthesis engine; schema_version 1.5 (bumped 1.4 to 1.5 in Feature 189 for additive list-of-RECORD field addition per ADR-028 Complex-Shape Addition Clarifier — extension of ADR-026 to list-of-RECORD fields; preceded by 1.3 to 1.4 bump in Feature 142; preceded by 1.2 to 1.3 bump in Feature 136); see `schemas/risk-scoring.yaml` for extension fields and [ADR-028](../02_ADRs/ADR-028-source-attribution-schema-extension.md) for the F-A2 contract |

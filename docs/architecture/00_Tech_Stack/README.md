...
|--------|---------|------------|
| `schemas/finding.yaml` | Intermediate Representation (IR) -- data contract between agents and templates; optional `scored_finding` extension for quantitative risk scoring (Feature 035); baseline-aware fields `delta_status` and `baseline_run_id` (Feature 074); MAESTRO layer classification `maestro_layer` (Feature 084); canonical CSA layer names for L5/L6/L7 (Feature 136); agentic pattern classification `agentic_pattern` (Feature 142); optional list-of-RECORD `source_attribution` field citing F-A1 external-framework IDs (Feature 189 F-A2); `OI-{N}` ID prefix for `output-integrity` threat-agent findings (Feature 201 F-1) | 15 fields: id, category, component, threat, likelihood, impact, risk_level, mitigation, references, dfd_element_type, maestro_layer (L1-L7 canonical CSA names or Unclassified, default Unclassified — L5 Evaluation and Observability, L6 Security and Compliance, L7 Agent Ecosystem per Feature 136), delta_status (NEW/UNCHANGED/UPDATED/RESOLVED), baseline_run_id (nullable), **agentic_pattern** (8-value enum per Feature 142: 6 canonical CSA MAESTRO patterns `agent_collusion`, `emergent_behavior`, `temporal_attack`, `trust_exploitation`, `communication_vulnerability`, `resource_competition` + `none` + `multiple`, default `none`), **source_attribution** (optional list-of-RECORD per Feature 189; each record `{taxonomy, id, relationship}` with closed 5-value `taxonomy` enum — `owasp` / `mitre-attack` / `mitre-atlas` / `nist-ai-rmf` / `cwe` external frameworks only — and closed 3-value `relationship` enum — `primary` / `related` / `derived` with default `primary`; field is absent when a finding cites no framework items; F-A2 ships the contract only, populator wiring deferred to F-A3 under 22-file zero-edit invariant; **first net-new producer is the `output-integrity` agent shipped in Feature 201 F-1** — every OI finding emits `source_attribution` citing OWASP LLM05:2025 primary + ≥1 CWE related); `id.pattern` regex extended in Feature 142 to accept `AGP-\d+` prefix for net-new findings emitted by the Phase 3.6 synthesis engine, extended again in Feature 201 to accept `OI-\d+` prefix for `output-integrity` agent findings (regex `^(S|T|R|I|D|E|AG|LLM|AGP|OI)-\d+$` — 10 alternation values); schema_version 1.6 (bumped 1.5 to 1.6 in Feature 201 for additive regex-alternation prefix extension per ADR-030 Decision 8 extending the ADR-026 Complex-Shape Clarifier from enum-value / list-of-RECORD additions to regex-alternation prefix additions; preceded by 1.4 to 1.5 bump in Feature 189; preceded by 1.3 to 1.4 bump in Feature 142; preceded by 1.2 to 1.3 bump in Feature 136); see `schemas/risk-scoring.yaml` for extension fields, [ADR-028](../02_ADRs/ADR-028-source-attribution-schema-extension.md) for the F-A2 contract, and [ADR-030](../02_ADRs/ADR-030-output-integrity-agent.md) for the F-1 OI prefix extension |
...
| `agents/ai/` | 6 agents | AI-specific threats: Prompt Injection, Tool Abuse, Data Poisoning, Model Theft, Agent Autonomy, **Output Integrity** (Feature 201 F-1 — OWASP LLM05:2025 Improper Output Handling; `OI-{N}` finding ID prefix; every finding emits `source_attribution` citing LLM05 primary + ≥1 CWE related); two-layer keyword dispatch (AG-prefixed agentic, LLM-prefixed LLM categories; LLM quartet dispatch rule in `dispatch-rules.md` per [ADR-030](../02_ADRs/ADR-030-output-integrity-agent.md)) | Validated end-to-end (Feature 007; Output Integrity added Feature 201) |

...
### KB-034: Byte-Identical Baseline Tests + Deterministic Timestamps Enable Same-Day Additive Schema Ships

**Date**: 2026-04-16
**Category**: Tooling insight
**Source**: Feature 142 delivery retrospective
**Severity**: Low (positive pattern)

**Problem**: Additive schema changes that touch a shared pipeline IR (finding.yaml bumps) historically carry a regression-fear tax — maintainers hedge by over-testing or deferring the ship. Without a fast, mechanical way to prove that non-targeted examples are untouched, each schema bump re-opens the question "did I silently change the other five outputs?" for every reviewer.

**Root Cause**: N/A — positive pattern observation. Feature 142 added an 8-value enum field (`agentic_pattern`) and a new orchestrator phase (3.6) in a single same-day ship against a Team-Lead 5-8d estimate. The compression was not heroics — it was infrastructure.

**Solution**: Pair **byte-identical baseline tests** (`tests/scripts/test_backward_compatibility.py` comparing generated PDFs against committed `.baseline` files per Feature 128) with **deterministic timestamp pinning** (`SOURCE_DATE_EPOCH=1700000000` per ADR-021) so that non-targeted examples produce byte-equal output across pipeline runs. Schema bumps that preserve the default-value invariant (e.g., new field defaulting to `none`) prove their backward-compat posture mechanically — if the bytes match, the invariant holds; if they don't, a diff pinpoints the drift. The targeted example (agentic-app) is regenerated on purpose; the other five are byte-frozen as a regression guard. Pytest fixtures (8 architecture YAML fixtures + 3 fixture subdirectories) further reduce the test-authoring tax so that 4 new test files (3,071 LOC) can land in the same wave as the implementation.

**Result**: Feature 142 shipped the `agentic_pattern` 1.3→1.4 schema bump, Phase 3.6 orchestrator synthesis, and 4 new test files in a single same-day delivery (estimate 1-2 days, actual same-day; on-target). 5 baselines byte-identical under `SOURCE_DATE_EPOCH=1700000000`; agentic-app regenerated by design per FR-015 Path 1. Zero-edit invariant on 11 STRIDE+AI detection agents preserved without a regression sweep. The backward-compat test was the single largest contributor to reviewer confidence — the SC claims on `threats.md` / `risk-scores.md` / `compensating-controls.md` across 5 examples were answered by `git diff` rather than inspection.

...
**Why it matters**: F-241's Stream 4 aggregator extension in `_load_framework_yaml_records()` added new `yaml.safe_load` call sites. Without the AST regression guard, a future refactor (e.g., "let's hoist the import for performance") would silently break stdlib-only callers. The test runs in <100ms — fast enough to be part of every test invocation. The pattern is generalizable: any feature that adds new dependency call sites should also extend the AST regression guard with the new module name.

...
|-----------|-------------|---------------|------------|
| End User | Medium (2.5) | — | 1 High + 1 Medium findings |
| WellnessCompanionChatbot | Medium (2.2) | — | 3 High + 7 Medium + 1 Low findings |
| Conversation Session Store | Medium (2.0) | — | 3 Medium findings |
| Interaction Audit Log | Medium (2.0) | — | 3 Medium findings |

**Risk Reduction**: Control coverage 0.0% (compensating-controls.md reports no `Found` controls — all 19 findings carry `No Control Found` status; this baseline is intentionally a clean-slate consumer-facing AI surface so the human-trust-exploitation pattern catalog emits without prior mitigations).
...
Spatial Architecture Diagram (16:9 landscape, white background):

1. **TITLE BAND (~10%)**: Project name and finding summary
2. **MAIN CANVAS (~80%)**: Trust zones rendered as colored rectangles containing component boxes; data flow arrows between components colored by severity; boundary crossings annotated; component badges show residual finding count + highest severity
3. **LEGEND PANEL**: Severity color key + finding ID quick-reference
...
White (#FFFFFF) — clean corporate report aesthetic.

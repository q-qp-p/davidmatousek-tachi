...
1. **TOP SECTION (~10%)**: Title, date, CONFIDENTIAL badge, subtitle with finding count
2. **MIDDLE ROW (~50%)**: Left panel (donut chart + risk posture), Center panel (heat map grid + cell-level grid), Right panel (top finding cards with colored left borders)
3. **BOTTOM STRIP (~30%)**: Architecture threat overlay listing components with risk weight and finding annotation

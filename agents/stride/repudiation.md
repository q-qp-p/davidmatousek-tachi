...
| `component` | Name of the External Entity or Process under analysis | `Payment Processing Service` |
| `threat` | Specific repudiation threat description — what action is deniable and why the system cannot prove it occurred | `User can deny initiating a refund because the refund endpoint logs the transaction ID but not the authenticated user identity or session context` |
...
| `mitigation` | Actionable countermeasure — specific logging enhancement, integrity control, or non-repudiation mechanism | `Extend refund endpoint logging to include authenticated user ID, session correlation ID, client IP, and timestamp; forward logs to append-only SIEM within 60 seconds` |
| `references` | OWASP, CWE, MITRE ATT&CK, or CVE identifiers supporting the finding | `["CWE-778", "OWASP A09:2021"]` |

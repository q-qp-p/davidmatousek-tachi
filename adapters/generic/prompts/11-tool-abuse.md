...
This is a standalone threat analysis prompt for the **Tool-Use Abuse** threat category.

**Input**: Provide the architecture components assigned to this agent by the orchestrator (from Step 11 of `00-orchestrator.md`).
**Output**: A list of findings in the structured format specified below.

**Conditional activation**: This agent only runs when architecture components match agentic/tool-related trigger keywords (see Detection Scope below). If the orchestrator determined no components match, skip this prompt entirely.

...
5. After all agents complete, return to this orchestrator context for Phase 3 (Countermeasures), Phase 4 (Assessment), Phase 5 (Report), and Phase 6 (Infographic).
...
- `13-threat-infographic.md` -- Visual risk infographic specification
...
6. **Phase 6 -- Infographic** (optional, default-on): Generate a visual risk specification and optional presentation-ready image.
...
1. Run the infographic prompt (file `13-threat-infographic.md`) with the completed `threats.md` as sole input.
...
**Pipeline isolation**: Phase 6 failures MUST NOT block or invalidate Phases 1-5 outputs. If the infographic prompt encounters any error — API failure, content policy rejection, schema validation failure, or unexpected exception — the pipeline completes successfully with all Phase 1-5 outputs intact. Phase 6 errors are logged but never propagated as pipeline failures.
...
2. **Fresh-context invocation**: Run the infographic prompt (file `13-threat-infographic.md`) in a fresh context. The invocation MUST:
   - Pass ONLY the `threats.md` file path as input
   - NOT pass accumulated pipeline state from Phases 1-5
   - NOT pass `threat-report.md`, `attack-trees/`, or any other Phase 5 outputs
   - NOT pass intermediate component inventories, agent dispatch logs, or correlation detection state
   - The infographic prompt reads `threats.md` and operates independently using only what that file contains
...
3. **Context isolation boundary**: The following content boundary applies to Phase 6:
   ...
   ```
   The infographic prompt treats `threats.md` as its complete input. It does not access or require any other pipeline artifacts. This prevents context window pressure from accumulated pipeline state and ensures the infographic prompt operates on the validated, final output.

4. **Output placement**: The infographic prompt writes its outputs to the same directory as `threats.md`:
   ```
   {output-directory}/
   ├── threats.md                    (Phase 4 — existing, unchanged)
   ├── threats.sarif                  (Phase 4 — existing, unchanged)

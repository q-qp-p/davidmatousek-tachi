...
into ``'AT{{PROJECT_NAME}}T'`` — the original F-248 regression class.
...
def test_template_substitute_shim_present() -> None:
    ...
    Removing it would re-introduce the F-248 'AT&T → AT{{PROJECT_NAME}}T'
    ...
    assert "shopt -u patsub_replacement" in helper_text, (
        ...
        "'AT{{PROJECT_NAME}}T' on bash 5.x runners — the original F-248 "

...
// Module-bound reference for safe field access on potentially-absent vars
// (used by F-194 / F-B coverage attestation §2b guards — handles the stale-
// data scenario where the wildcard import above has no binding to fall back on).
...
// Coverage attestation defaults (F-194 / F-B — false + empty = no coverage attestation section).
// Uses dictionary(module).at(key, default: ...) so stale report-data.typ files
// generated by pre-F-B extract-report-data.py (where the F-B vars are entirely
// absent — not just `none`) compile cleanly. Wildcard-import + `if X != none`
// guard pattern (used elsewhere in this block) only handles the present-but-
// none case, not the absent-from-import case.
...
// --- Coverage Attestation (conditional, F-194 / F-B) ----------------------
// Renders per-finding attribution table + 5 per-framework matrix pages when
// any finding carries source_attribution (gate). On all 6 current baselines
// the gate is false (F-A3 populator hasn't shipped) so this block is omitted
// and PDFs remain byte-identical to pre-F-B (SC-002).
